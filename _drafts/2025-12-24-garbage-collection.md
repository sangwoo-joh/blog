...
### 박싱 (Boxing)
가비지 찾기 단계를 통해 객체들을 구분하고 나면 가비지 수집 단계에서 이것들을 알아야 한다. 즉, 가비지 찾기 단계가 완료되고 나면, 각각의 객체에 대해서 "얘는 가비지임" 혹은 "얘는 가비지 아님"을 알 수 있어야 한다. 이게 가능하려면 해시 테이블에 객체마다 메타데이터를 기록하던지, 아니면 객체마다 추가로 메타데이터를 씌워서 기록하던지 둘 중 하나밖에 없다. 해시 테이블을 유지하는 비용이 크고 캐시 친화적이지 못하기 때문에 대부분의 가비지 컬렉션은 후자의 방법, 즉 객체마다 추가로 메타데이터를 직접 들고 있는 방식을 택하는데 이를 **박싱**이라고 한다.
...
트레이싱 컬렉션은 루트 집합으로부터 시작해서 닿을 수 있는 모든 객체들을 탐색하는 일종의 그래프 탐색이라고 이해할 수 있다.

## 공통 개념들
트레이싱 컬렉션에서 주로 쓰이는 몇 가지 공통적인 개념들이 있는데 이것들을 알아두면 좋다.
