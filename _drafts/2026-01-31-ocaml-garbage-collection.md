...
[가비지 컬렉션](../garbage-collection) 중 하나인 트레이싱 컬렉션이 현실의 프로그래밍 언어에서 어떻게 구현되어있는지를 OCaml 4.14 버전[^1], 64비트 `amd64` 리눅스를 기준으로 살펴보려고 한다.


[^1]: OCaml 5.x 부터는 GIL이 사라지고 이론과 연구와 엔지니어링이 뒷받침된 진정한 의미의 멀티코어 가비지 컬렉션이 도입되었는데, 이건 또 다른 엑스트라 라지 톡이 필요한 이야기라 뒤로 미루고, 일단은 GIL이 있는 싱글코어 가비지 컬렉션을 기준으로 살펴보기 위해서 4.x의 최신 버전인 [4.14.2](https://github.com/ocaml/ocaml/tree/4.14.2)를 골랐다.

# OCaml의 가비지 컬렉션
OCaml은 다음과 같은 가비지 컬렉션을 구현하고 있다.
 * 박싱: OCaml은 [메모리 주소의 특징](../virtual-memory#주소는-항상-짝수)을 이용해서 일반 정수 타입과 박싱된 타입을 구분한다.
 * 세대 별 힙 & 컬렉션: 마이너 힙과 메이저 힙 두 세대를 운영하고 있다. 각각의 힙은 서로 다른 방법으로 컬렉션된다.
 * 복사 컬렉션: 마이너 힙은 체이니의 복사 컬렉션 방법을 구현하고 있다. 할당은 아주 빠른 포인터 덧셈 연산 (Bump Pointer Allocation) 으로 이루어지고, 컬렉션은 STW 조정을 통해 이루어 진다.
 * 마크-스윕-컴팩트 컬렉션: 메이저 힙은 마크 & 스윕 & 컴팩트 컬렉션으로 구현된다. 기본적으로는 마크 & 스윕이 진행되고, 임계점을 넘으면 컴펙션이 진행된다.
 * 점진적 컬렉션: 메이저 힙 컬렉션(특히 마킹)은 STW로 한번에 진행되지 않고 작업을 조금 씩 잘라서 증분적으로 수행된다.
 * 쓰기 배리어와 SATB: 메이저 힙의 마킹 단계의 올바름과 결정가능성을 위해서 OCaml의 컴파일러가 모든 포인터 연산에 쓰기 배리어(정확히는 유아사의 삭제 배리어)를 도입하고 추가로 메모리 할당을 검은색으로 하여 SATB 불변식을 유지한다.
 * 세대 별 쓰기 배리어: 메이저 힙에서 마이너 힙으로 가는 모든 포인터 쓰기를 추적하여 세대 간 참조를 기억집합에 추적한다.
 * 프리 리스트: 메이저 힙 메모리는 프리 리스트를 통해 관리된다.
 * 이페메론: OCaml은 언어 네이티브 수준에서 이페메론이 구현되어 있다. 이페메론이 어떻게 GC와 상호작용하는지 살펴볼 수 있다.

수십년 간 쌓아온 연구의 결과물이라서 모든 배경을 이해하는 일이 쉽지는 않다. 하지만 LLM의 시대인 만큼 의지만 있다면 확실하게 거인의 어깨에 올라서서 더 넓은 세상을 바라볼 수 있다.

그럼 차근차근 하나씩 살펴보자.

# 디렉토리 구조
이런 대규모 프로젝트는 보통 루트 디렉토리에서 `tree -d`로 일단 디렉토리 구조부터 살펴보면 대충 내가 원하는 바를 이해하기 위해서는 어떤 코드를 살펴봐야 할지 감을 잡을 수 있다. 전부 다 볼 필요는 없고 몇 가지 흥미로운 것만 살펴보자면:
 * `HACKING.adoc`: OCaml 코드를 해킹하기 위한 설명서다. 대규모 프로젝트에 이런 문서가 있으면 감사한 마음을 가지고 정독해야 한다. 보통 디렉토리 구조에 대한 설명, 기여하는 방법, 개발 팁 등이 적혀 있다. 특히 OCaml 컴파일러에 있는 OCaml 코드의 타입을 Merlin으로 살펴보는 방법이 있어서 큰 도움이 되었다.
 * `driver`: 컴파일러의 메인 함수이자 진입점을 드라이버라고 부른다.
 * `boot`: 컴파일러 부트스트래핑을 위한 모듈이다. OCaml 컴파일러는 OCaml로 작성되어 있어서 미리 빌드된 버전을 갖고 있다.
 * `parsing`: 파서가 구현되어 있다. OCaml 문법으로 작성된 코드를 AST로 변환한다.
 * `typing`: 타입 체커가 구현되어 있다. 파싱한 AST에 대해서 Hindley-Milner 타입 추론 알고리즘을 이용하여 타입 검사를 수행하고 타입이 있는 표현식을 생성한다.
 * `lambda`: OCaml의 중간 언어인 람다가 구현되어 있다.
 * `middle_end`: 람다 IR 위에서 할 수 있는 최적화가 구현되어 있다.
 * `asmcomp`: 네이티브 컴파일러의 코드 생성을 위한 모듈이다. 안에 보면 `amd64`, `arm64`, 심지어는 `power`나 `riscv` 아키텍쳐까지 있다.
 * `bytecomp`: 바이트코드 생성 모듈이다. 바이트코드의 명령어와 실행 시맨틱이 구현되어 있다.
 * `stdlib`: 표준 라이브러리가 구현되어 있다.
 * `otherlibs`: 표준 라이브러리 외에 운영체제마다 추가로 사용할 수 있는 라이브러리가 구현되어 있다. 예를 들면 `unix`, `win32unix`, `systhreads`, 그리고 정규식 관련 코드가 있다.
 * `runtime`: OCaml 프로그램이 실행되는 환경이 구현되어 있다.

가비지 컬렉션을 이해하기 위해서 봐야 할 모듈은 `runtime`이다. 이 디렉토리 안을 보면 파일 이름만 봐도 여기구나 싶은 것들이 많다. 예를 들면, `freelist.c`, `minor_gc.c`, `major_gc.c` 등이 있고, `runtime/caml` 에는 헤더 파일들이 모아져 있다. OCaml의 창시자 Xavier Leroy의 [ZINC](https://caml.inria.fr/pub/papers/xleroy-zinc.pdf) 페이퍼를 보면 "...바이트코드 인터프리터가 C와 같은 인기 있는 고급 언어로 작성되면 C 컴파일러가 있는 모든 기계에서 거의 또는 전혀 수정없이 실행할 수 있습니다. ..." 라는 언급이 있는데, 덕분에 많은 부분이 C 언어로 작성되어 있다.
