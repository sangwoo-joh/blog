...
즉, 뮤테이터와 컬렉터가 동시에 동작하는 경우에는 마킹 페이즈의 정확성이 깨져버린다. 그래서 이를 막기 위해서 마크 스윕 컬렉션은 그 악명 높은 STW(Stop-The-World) 조정을 도입한다. 컬렉터가 일을 하는 동안 프로그램(뮤테이터)을 멈추는 것이다. 좀더 정확하게는, 컬렉터가 마킹 페이즈에 도입해서 모든 객체의 그래프의 마킹을 완료할 때까지 뮤테이터를 완전히 멈춰서 색깔이 잘못 칠해지는 것을 막는다. 스위핑 페이즈에서는 이미 마킹 페이즈에서 안전하게 가비지와 살아있는 객체를 구분했기 때문에 멈출 필요는 없다.

### Incremental Collection
올바른 가비지 컬렉션을 위해서 STW가 필요하다는 것은 자명하다. 하지만 마크 스윕 컬렉션이 처음 Lisp에 도입됐던 그 당시의 하드웨어 한계로 인해서 프로그램이 수 밀리초에서 수 초간 멈춰버리는 것이 비일비재했는데, 이로 인해서 사용자 경험이 좋지 못했다. 게다가 점차 하드웨어가 발전하면서 메모리의 용량도 늘어났는데, 마크 스윕 컬렉션의 경우 관리되는 메모리 전체를 봐야하기 때문에 역설적으로 메모리 용량이 증가할수록 멈춰야 하는 시간이 늘어날지도 모른다. 그래서 초기의 마크 스윕 컬렉션은 응답성이 좋지 못해서 실시간 어플리케이션에 적합하지 못하다는 의견이 많았다.

이걸 해결하기 위해서 도입한 것이 바로 점진적 컬렉션(Incremental Collection)이다. 마킹과 스위핑 모두 한꺼번에 진행하는게 아니라 작업을 조금씩 잘라서 (Slicing) 점진적으로 수행해서 (Incremental) 사용자 프로그램이 너무 긴 정지 시간 (Pause Time)을 겪지 않도록 해서 반응성을 챙기는 것이다. 정확하게는 "예측할 수 있는" 정지 시간을 제한할 수 있도록 해서 가비지 컬렉션의 작업과 사용자 프로그램의 반응성(멈추는 시간) 사이의 트레이드 오프를 조절할 수 있게 되었다.

점진적 컬렉션 덕분에 트레이싱 컬렉션도 실시간 어플리케이션에서 사용할 수 있을 정도의 반응성을 얻게 되었다.

#### Write (Read) Barrier
점진적 컬렉션 덕분에 실시간 반응성을 얻기 되었지만, 한 가지 문제가 있었다. 바로 새로운 불변 조건 (Invariant)가 생긴 것이다: **"검은색 객체가 흰색 객체를 가리키면 안된다."**

앞의 세 가지 색깔 스킴을 다시 떠올려보자. 점진적 컬렉션에서는 컬렉터가 열심히 마킹하다가 잠깐 멈추고 뮤테이터가 객체 상태를 변경할 수 있다. 이때 예를 들어, A 가 검은색으로 칠해졌고 그 자식들이 회색으로 칠해졌다고 하자. 뮤테이터가 A -> C(검은색 -> 회색)로 포인터가 있던 걸 B -> D (둘 다 흰색)랑 교환한다고 하자. 이러면 A(검은색)은 D(흰색)를 가리키게 되고, B(흰색)은 C(회색)를 가리키게 된다. 그리고 D를 가리키는 유일한 포인터는 A가 된다. C는 B(흰색)에 의해 다시 Reachable 하게 되고, D는 A 외에는 포인터가 없는데 A가 이미 검은색(그래프 색칠하는 탐색이 끝났음)이라서 절대 닿을 수 없게 된다. 이러고 GC Marking이 끝나면 Sweeping 때 D는 흰색이라서 가비지로 간주되어 회수되는데 원래는 회수되면 안되는 애다. 그래서 이 invariant가 필요하다.

// 그림 필요

이걸 지키기 위해서 모든 포인터 연산에 배리어를 도입할 수 있다. 이때 쓰기 연산에 배리어를 도입하면 쓰기 배리어 (Write Barrier), 읽기 연산에 도입하면 읽기 배리어 (Read Barrier)라고 하는데 보통은 쓰기 배리어가 더 많이 적용되어 있는 것 같아 여기서도 쓰기 배리어를 중심으로 이야기를 풀어나가려고 한다.

아무튼 쓰기 배리어는 **모든** 포인터 쓰기 연산이 일어날 때 추가적으로 메모리를 관리하기 위한 코드를 실행하는 메커니즘이다. 모든 포인터 연산을 추적하고 자동으로 이 메커니즘을 삽입하려면 가비지 컬렉션 로직 만으로는 부족하고, 컴파일러의 도움을 받아야 한다. 그래서 쓰기 배리어까지 적용된 완전한(Complete) 가비지 컬렉션은 라이브러리나 패키지가 아니라 프로그래밍 언어 수준에서 구현될 수 밖에 없다.

쓰기 배리어의 역할은 좀 헷갈릴 수도 있다. 왜냐하면 점진적 (혹은 동시성) 가비지 컬렉션에서의 역할과, 아래에서 설명할 세대 별 가비지 컬렉션에서의 역할이 미묘하게 다르기 때문이다. 일단 여기서는 점진적 컬렉션에서의 역할을 살펴보자.

점진적 컬렉션에서는 뮤테이터와 커렉터가 동시에 실행될 수 있다. 여기서의 불변식은 "검은색 객체가 흰색 객체를 가리키면 안된다"는 것이다. 만약 뮤테이터가 이미 마킹된 검은색 객체가 흰색 객체를 가리키도록 수정해버리면
...
그러면 이거 실제로 구현은 어떻게 할까? 진짜로 마킹 시작할 때 뮤테이터를 멈춘 다음에 힙 메모리 전체의 상태를 스냅샷으로 찍듯이 어딘가에 저장하는 구현은 대단히 비효율적일 것이다. 그래서 많은 프로그래밍 언어들이 실제로는 대단히 기발한 방법을 구현한다. 예를 들어 OCaml의 경우, 컬렉터가 **마킹 중일 때 할당하는 모든 객체는 미리 검은색으로 칠해버린다**. 이렇게 하면 이후 스위핑 페이즈에서 컬렉터가 마킹 중에 할당한 모든 객체를 이미 마킹했다고 여겨, 마치 마킹 초기에 스냅샷을 찍은 것과 같은 효과를 볼 수 있다.
